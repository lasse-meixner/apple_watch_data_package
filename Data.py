...
redundant = ["device","sourceName","HKMetadataKeyHeartRateRecoveryActivityType","HKHeartRateEventThreshold","HKWorkoutEventTypeMarker","HKMetadataKeyUserMotionContext","HKMetadataKeySyncIdentifier","HKMetadataKeySyncVersion","HKMetadataKeyHeartRateRecoveryTestType","HKMetadataKeyHeartRateRecoveryActivityDuration","HKMetadataKeyHeartRateRecoveryMaxObservedRecoveryHeartRate","HKMetadataKeySessionEstimate"]

def add_date_variables(data):
    if data.startDate.isna().sum() > 0:
        print(f"Dropping {data.startDate.isna().sum()} entries with missing time")
        data.dropna(subset=["startDate"],inplace=True)
        
    data["start"] = [datetime.strptime(x[:-6],"%Y-%m-%d %H:%M:%S") for x in data.startDate]
    data["end"] = [datetime.strptime(x[:-6],"%Y-%m-%d %H:%M:%S") for x in data.endDate]
...
def add_workout_variables(data):
    ...
    for i, workout in tqdm(workouts.iterrows()):
        ...
        workout_type = workout.workoutActivityType[workout.workoutActivityType.find("Type")+4:].lower()
        identifier = workout_type + str(workout.start)
        pause_start = datetime.strptime(workout.HKWorkoutEventTypePause[:-6],"%Y-%m-%d %H:%M:%S") if isinstance(workout.HKWorkoutEventTypePause,str) else pd.NA
        pause_end = datetime.strptime(workout.HKWorkoutEventTypeResume[:-6],"%Y-%m-%d %H:%M:%S") if isinstance(workout.HKWorkoutEventTypeResume,str) else pd.NA
        ...
        indices = ((data["start"] > workout.start) & (data["end"] <= workout.end))
        ...
        workout_subset["duration_into_workout"] = (workout_subset["start"]-workout.start).dt.total_seconds()/60
        workout_subset["net_duration"] = workout.duration # this minuses out the time spend in pause
        workout_subset["all_duration"] = workout.end - workout.start
        workout_subset["identifier"] = identifier
        new_cols = ["workout", "workout_type", "duration_into_workout", "net_duration","all_duration", "identifier"]
        ...
        if has_pause_start:
            ...
            if has_pause_end:
                workout_subset["paused"] = (workout_subset["start"]>pause_start) & (workout_subset["end"]<pause_end)
                workout_subset["duration_since_last_paused"] = [max(0,x) for x in (workout_subset["start"]-pause_start).dt.total_seconds()/60]
                new_cols.append("duration_since_last_paused")
            else: workout_subset["paused"] = (workout_subset["start"]>pause_start) & (workout_subset["end"]<workout.end)
...
def modify_units(data):
    data["HKMetadataKeyBarometricPressure"] = data["HKMetadataKeyBarometricPressure"].str.extract("(\d+)").astype(float)
    data["HKElevationAscended"] = data["HKElevationAscended"].str.extract("(\d+)").astype(float)/100 # -> in meters
    data["HKAverageMETs"] = data["HKAverageMETs"].str.extract("(\d+)").astype(float)# -> kcal/hr*kg
    data["HKWeatherHumidity"] = data["HKWeatherHumidity"].str.extract("(\d+)").astype(float)/100 # -> in %
    data["HKWeatherTemperature"] = (data["HKWeatherTemperature"].str.extract("(\d+)").astype(float)-32)*(5/9) # -> in °C
    return data
...
def preprocess(data,drop=False,set_start_index=False):
    ...
    if drop: 
        data.drop(axis=1,labels=redundant,inplace=True) # the list needs to be updated
...
def get_heartrate_data(data):
    ...
    hr["first_on_wrist"] = ((hr["off_wrist"] > 5) & (hr["time"]<time(13)) & (hr["time"]>time(4)))
